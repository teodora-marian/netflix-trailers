export async function getLikedVideos(userId, jwtToken) {
  const operationsDoc = `
  query likedVideos($userId: String!) {
    netflixTrailers_stats(where: {userId: {_eq: $userId}, liked: {_eq: 1}}) {
      videoId
      }
    }
  `;
  ...
  console.log("likedVideosHasura (f) response", response);
  return response?.data?.netflixTrailers_stats;
}

export async function getWatchedVideos(userId, jwtToken) {
  const operationsDoc = `
  query watchedVideos($userId: String!) {
    netflixTrailers_stats(where: { watched: {_eq: true}, userId: {_eq: $userId} }) {
      videoId
      }
    }
  `;
  ...
  console.log("watchedVideosHasura (f) response", response);
  return response?.data?.netflixTrailers_stats;
}
